...
function DocumentDetailPageContent() {
  ...
  const documentRequestRef = useRef<{
    id: string;
    promise: Promise<KnowledgeDocument>;
  }>();
  ...
  useEffect(() => {
    let cancelled = false;
    // Strict Mode 会重放 effect；同一 id 复用进行中的请求，避免发出重复 HTTP 请求。
    const existing = documentRequestRef.current;
    const request =
      existing?.id === id ? existing.promise : documentService.get(id);
    documentRequestRef.current = { id, promise: request };
    void request
      .then((document) => {
        if (!cancelled) setDoc(document);
      })
      .catch((e: unknown) => {
        if (documentRequestRef.current?.promise === request)
          documentRequestRef.current = undefined;
        if (!cancelled)
